...
// AppContent component that uses EventContext and implements key-based remounting
function AppContent(): JSX.Element {
  ...
  return (
    <Suspense fallback={<LoadingFallback />}>
      <Routes>
        {/* Public routes - not keyed, persist across event changes */}
        ...
        {/* Protected scoring routes - keyed to remount on event change */}
        <Route path="/score/:cageNumber" element={
          <ProtectedRoute requiredRoles={['judge', 'admin']}>
            <ScorePage key={activeEventId} />
          </ProtectedRoute>
        } />

        {/* Protected cage scoring routes - keyed to remount on event change */}
        <Route path="/cage-score/:cageNumber" element={
          <ProtectedRoute requiredRoles={['judge', 'admin']}>
            <CageScorePage key={activeEventId} />
          </ProtectedRoute>
        } />

        {/* Protected class scoring routes - keyed to remount on event change */}
        <Route path="/class-score/:catId" element={
          <ProtectedRoute requiredRoles={['judge', 'admin']}>
            <ClassScorePage key={activeEventId} />
          </ProtectedRoute>
        } />
        ...
        {/* Protected fit and show scoring routes - keyed to remount on event change */}
        <Route path="/fit-show-score/:catId" element={
          <ProtectedRoute requiredRoles={['judge', 'admin']}>
            <FitShowScorePage key={activeEventId} />
          </ProtectedRoute>
        } />
        ...
        {/* Class score management route - admin only, not keyed */}
        ...
        {/* Protected sign page - admin utility, not keyed */}
        ...
        {/* Protected admin routes with layout - keyed for remounting */}
        <Route path="/" element={
          <Authenticator>
            {({ signOut }) => {
              console.log('Authenticator rendering for admin routes');
              return <AppLayout key={activeEventId} signOut={signOut || (() => { })} />;
            }}
          </Authenticator>
        }>
