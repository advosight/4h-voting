...
const CREATE_JUDGE_ACCOUNT = `
  mutation CreateJudgeAccount($input: CreateJudgeInput!) {
    createJudgeAccount(input: $input) {
      userId
      email
      name
      judgeId
  ...
  const handleCreateJudge = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await client.graphql({
        query: CREATE_JUDGE_ACCOUNT,
        variables: { input },
      });

      // Reset form and reload judges
      setFormData({ 
        email: '', 
        name: '', 
        temporaryPassword: '', 
      ...
      setError(err instanceof Error ? err.message : 'Failed to create judge account');
    } finally {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        >
          {showCreateForm ? 'Cancel' : 'Create Judge Account'}
        </button>
      </div>
        ...
        <div style={{
          ...
          <form onSubmit={handleCreateJudge}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              ...
              >
                {createLoading ? 'Creating...' : 'Create Judge'}
              </button>
              <button
                type="button"
      ...
      <div style={{ backgroundColor: 'white', border: '1px solid #dee2e6', borderRadius: '4px' }}>
        ...
        {judges.length === 0 && (
          <div style={{ padding: '2rem', textAlign: 'center', color: '#6c757d' }}>
            No judge accounts found. Create one to get started.
          </div>
        )}
      </div>
    </div>
