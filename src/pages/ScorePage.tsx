...
function ScorePage(): JSX.Element {
  ...
  const initializePage = async () => {
    try {
      ...
      try {
        ...
        logError(authError, 'ScorePage.initializePage.auth');
      ...
      console.error('ScorePage initialization error:', err);
      if (err && typeof err === 'object' && 'errors' in err) {
        console.error('GraphQL errors:', err.errors);
      }
      logError(err, 'ScorePage.initializePage');
  ...
  const handleSave = async (scoreData: CreateScoreInput) => {
    try {
      setSaving(true);
      setError('');
      ...
      // Refresh the page data to show the saved score
      await initializePage();
      
    } catch (err) {
      logError(err, 'ScorePage.handleSave');
      const parsedError = parseError(err);
  ...
  const handleSubmit = async (scoreData: CreateScoreInput) => {
    try {
      setSaving(true);
      setError('');
      ...
      await handleOptimisticLockConflict(
        async () => {
          const submitWithRetry = withRetry(async () => {
            return await client.graphql({
              query: createScore,
              variables: { input: { ...scoreData, isFinalized: true } }
            });
          }, { maxRetries: 2 });
      ...
      // Navigate back to admin dashboard or show success message
      navigate('/', { 
        state: { 
          message: `Score submitted successfully for ${cat?.name} in cage ${cageNumber}!` 
        }
      });
      
    } catch (err) {
      logError(err, 'ScorePage.handleSubmit');
      const parsedError = parseError(err);
        ...
        <AppBar position="static" elevation={2}>
          <Toolbar>
            ...
            <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
              4H Cat Scoring - Cage {cageNumber}
            </Typography>
            <Button
              color="inherit"
            ...
            <CardContent>
              ...
              {existingScore && (
                <Alert severity="warning" sx={{ mt: 2 }}>
                  <strong>Note:</strong> You have already scored this cat. 
          ...
          <Card elevation={3}>
            <CardContent>
              <ScoringForm
                ...
                loading={saving}
                hasPermission={hasScorePermission}
              />
            </CardContent>
          </Card>
